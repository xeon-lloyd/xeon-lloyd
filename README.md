<div>
  <img height="150" src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/df148a57-41fd-4236-abbe-ae0fb466b8a7"  />
</div>


<h1>소스코드 적는 여우 🦊</h1>

*잘 됐으면 좋겠다*  <br><br>

<h3>👩‍💻  About Me</h3>

> 혼자서 외롭게 개발하는 고독한 여우 개발자

As Known As **"무능한 제온씨"**

<br>

2017년부터 지금까지 쭉 개발을 하고 있습니다<br>
주로 웹 서비스 개발을 하고있으며<br>
지금은 학교를 다니면서 소소하게 서비스를 개발하고 운영하고 있어요!<br>
<br>

근데 사실 웬만한 분야는 다 찍먹해봐서 이것저것 아는! 이에요<br>
<br>

**포트폴리오** : https://portfolio.ritchko.com  
<br>
<br>

* * *

<h2>⛏️ 하고 있는거</h2>


<h3>서비스 운영</h3>

**=개인 프로젝트**  
저는 스스로 **"IT 자영업자"** 라고 말하고 다닙니다.  
프로젝트를 통해서 서비스를 만들고 운영하는 것을 좋아합니다.

<table>
	<tr>
		<td>
			<img src="https://tewind.app/img/logo.png" height="30"><br>
			<b>Te:wind</b><br>

> https://tewind.app<br>
트위치 방송 녹화 서비스<br>
2024.02 ~ 

> *~~https://tewind.kr~~* *(운영 종료)*<br>
트위치 다시보기 서비스<br>
2023.04 ~ 2024.02

```ffmpeg``` ```hls``` ```카드 정기결제```  
```nodejs``` ```aws``` ```s3``` ```cloudflare/workers```  
```vanilla js/css```

트위치가 한국에서 다시보기 서비스를 종료하여<br>
이를 대체할 서비스가 있으면 좋겠다고 느껴 개발/운영중<br>
<br>
그 와중에 트위치가 2024년 2월 한국에서 철수해버려서 방향을 조금 비틀어서<br>
키리누커나 팬영상 제작자를 위한 클라우드 녹화 서비스로 바꿔서 운영<br>
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=6f4777f0c1d24e238d27bcf756833238&pm=c)
<br>
</td>
	</tr>
	<tr>
		<td>
			<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/52e421a1-4abf-423f-b645-07425e3b8bd7" height="30"><br>
			<b>cricrip</b><br>

> <i>(운영 종료)</i><br>
트위치 실시간 클립 생성 서비스<br>
2022.11 ~ 2023.03

```puppeteer``` ```hls```  
```nodejs``` ```aws```  
```vanilla js/css```

위 티와인드과 같은 맥락으로 트위치가 한국에서의 클립 서비스 종료를 발표하고<br>
*어? 대체 서비스 해볼만 하겠는데?* 생각해서 개발<br>

방송을 짧게 녹화하고 있다가 실시간으로 클립을 생성할 수 있도록 만든 서비스

해당 서비스는 <b>스티키밤</b>에서 인수해서 ~~운영중! (A.K.A. 스티키밤 클립)~~<br>
스티키밤에 합류해서 함께 구축<br>
*트위치 철수사태로 운영 종료*<br>
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=2183fa8adaf943ccac710a658adb566c&pm=c)
<br>
</td>
	</tr>
	<tr>
		<td>
			<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/f744169d-9389-4c37-966d-f66a7c0a2c74" height="30"><br>
			<b>YukaiGames</b><br>

> https://yukaigames.com<br>
웹게임 엔진/제작/유통 플랫폼<br>
2017.12 ~ 2022.08 *(우선순위에서 밀려 방치중)*

```three.js```  
```nodejs``` ```s3```  
```vanilla js/css```

웹에서 클라우드 환경으로 설치 없이 게임 제작, 유통, 플레이를 할 수 있는 통합 플랫폼<br>
고등학생 때부터 하고 있는 인생을 건 장기 애착 프로젝트.<br>
그러나 혼자서 감당할 수 없는 너무나 거대한 프로젝트 규모와 다른 프로젝트에 집중하기 위해서 유기된...<br>
*~~(언젠가 다시 잡을 날이 오길..?)~~*<br>
<br>
이거 만들면서 지금 실력의 대부분을 만들 수 있었습니다😁  
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=4d1e6bd78c594437b423c2ffd7a96eae&pm=c)
<br>
</td>
	</tr>
	<tr>
		<td>
			<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/e5941299-3e39-4f5f-ab32-85452262c351" height="30"><br>
			<b>marshmelody</b><br>

> https://marshmelody.net<br>
음악 공유 플랫폼<br>
2017.03 ~ 2019.08 *(리뉴얼 예정)*

```php``` ```vanilla js/css``` 

개발을 시작하고 처음 만들었던 프로젝트이자 서비스<br>
이 서비스를 만들고 운영하면서 웹 개발에 대한 노하우를 쌓음<br>
서비스는 운영되지만, 유기한지 오래되어서 약간 부족한게 많은 부끄러운 프로젝트가 됨<br>
**리뉴얼할 타이밍 엿보는 중**  
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=8bb53f2fe5cd45c991738604a81c79af&pm=c)
<br>
</td>
	</tr>
</table>
<b>위 서비스 다 혼자서 개발하고 운영중인가요?</b>
<div> - 네, 별도의 인원없이 모두 제가 개발하고 운영하고 있습니다!</div>


<br>
<h3>🧰 외부(?) 프로젝트</h3>

**=외주**

<table>
	<tr>
		<td>
			<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/aec3aef2-da5a-4de3-98a9-999f1f5918e8" height="30"><br>
			<b>재고블럭 API</b><br>

> 재고블럭:재고 관리의 새로운 패러다임<br>
2022.11 ~ 2023.02

```nodejs```

재고 흐름을 관리하는 서비스의 백엔드, API 제작  
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=0ad5b960dc1d456eaeff8a113aaf39ae&pm=c)
<br>
</td>
	</tr>
	<tr>
		<td>
			<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/aec3aef2-da5a-4de3-98a9-999f1f5918e8" height="30"><br>
			<b>재고블럭 POS기 가상 프린터 클라이언트</b><br>

> 영수증 수집을 위한 POS에 설치하는 가상프린터 클라이언트<br>
2023.09 ~ 2023.11

```c#``` ```com0com```

가상프린터 드라이버와 C#을 이용해서 POS기의 영수증을 수집, 서버로 전송<br>
윈도우 10 기반의 POS기에 설치하는 클라이언트 프로그램 제작  
<br>
[>> 프로젝트 더 알아보기](https://succinct-pound-734.notion.site/IT-49bc4fc2d693408ba0e208b1073b2137?p=064318827fce4d92bb946678685f07a8&pm=c)
<br>
</td>
	</tr>
</table>

<br>
<br>

* * *


<h2>🔧 할 줄 아는거</h2>


<h3>개발(주력/웹 프로젝트)</h3>

<h4>FRONTEND</h4>
<table>
	<tr>
		<td align="center">
  			<img stype="display: inline-block;" src="https://cdn.simpleicons.org/html5/E34F26" height="35" alt="html5 logo"/>
			<div style="text-align: center;">HTML</div>
		</td>
		<td align="center">
  			<img src="https://cdn.simpleicons.org/css3/1572B6" height="35" alt="css3 logo"/>
			<div style="text-align: center;">CSS</div>
		</td>
		<td align="center">
			<img src="https://cdn.simpleicons.org/javascript/F7DF1E" height="35" alt="javascript logo"  />
			<div style="text-align: center;">javascript</div>
		</td>
		<td align="center">
			<img src="https://skillicons.dev/icons?i=ps" height="35" alt="adobephotoshop logo"  />
			<div style="text-align: center;">PhotoShop</div>
		</td>
		<td align="center">
			<img src="https://skillicons.dev/icons?i=ai" height="35" alt="adobeillustrator logo"  />
			<div style="text-align: center;">Illustrator</div>
		</td>
	</tr>
</table>


<h4>BACKEND</h4>
<table>
	<tr>
		<td align="center">
  			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-plain-wordmark.svg" height="35" alt="nodejs logo"  />
			<div style="text-align: center;">Node.js</div>
		</td>
		<td align="center">
  			<img src="https://skillicons.dev/icons?i=express" height="35" alt="express logo"  />
			<div style="text-align: center;">express</div>
		</td>
		<td align="center">
			<img src="https://cdn.simpleicons.org/mysql/4479A1" height="35" alt="mysql logo"  />
			<div style="text-align: center;">mysql</div>
		</td>
		<td align="center">
			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" height="35" alt="redis logo"  />
			<div style="text-align: center;">redis</div>
		</td>
		<td align="center">
			<img src="https://static-00.iconduck.com/assets.00/aws-s3-simple-storage-service-icon-423x512-sofvbo3x.png" height="35" alt="s3 logo"  />
			<div style="text-align: center;">s3</div>
		</td>
		<td align="center">
			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/socketio/socketio-original.svg" height="35" alt="socketio logo"  />
			<div style="text-align: center;">socket.io</div>
		</td>
	</tr>
</table>


<h4>CLOUD/INFRA</h4>
<table>
	<tr>
		<td align="center">
  			<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/93/Amazon_Web_Services_Logo.svg/768px-Amazon_Web_Services_Logo.svg.png" height="35" alt="amazonwebservices logo"  />
			<div style="text-align: center;">AWS</div>
		</td>
		<td align="center">
  			<img src="https://seeklogo.com/images/V/vultr-logo-B513C27910-seeklogo.com.png" height="35" alt="vultr logo"  />
			<div style="text-align: center;">vultr</div>
		</td>
		<td align="center">
  			<img src="https://cdn.simpleicons.org/cloudflare/F38020" height="35" alt="cloudflare logo"  />
			<div style="text-align: center;">Cloudflare</div>
		</td>
		<td align="center">
			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="35" alt="github logo"  />
			<div style="text-align: center;">github</div>
		</td>
		<td align="center">
			<img src="https://cdn.simpleicons.org/githubactions/2088FF" height="35" alt="githubactions logo"  />
			<div style="text-align: center;">github action</div>
		</td>
		<td align="center">
			<img src="https://cdn.simpleicons.org/raspberrypi/A22846" height="35" alt="raspberrypi logo"  />
			<div style="text-align: center;">raspberry PI</div>
		</td>
		<td align="center">
			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ubuntu/ubuntu-plain.svg" height="35" alt="ubuntu logo"  />
			<div style="text-align: center;">Ubuntu</div>
		</td>
	</tr>
</table>

<h4>ETC</h4>
<table>
	<tr>
		<td align="center">
  			<img src="https://img.icons8.com/?size=256&id=32418&format=png" height="35" alt="ffmpeg logo"  />
			<div style="text-align: center;">FFMPEG</div>
		</td>
	</tr>
</table>



<h4>❗잉? 프레임워크 안써요?</h4>

 - 네, 따로 프레임워크를 사용하지 않고 순수 CSS, JS(Front/Back)에서 개발합니다. *~~(그래서 취업 시장에서는 싫어하더라고요...ㅠ)~~*
 - 대신 자체적으로 정리해서 만든 라이브러리가 있어요. 이걸로 생산성을 높였어요
   - 웹 프로젝트 템플릿 : <a href="https://github.com/xeon-lloyd/ritchko-template">ritchko-template</a>

<br>

<h3>다른거</h3>


<h4>개발(할 줄 알아요)</h4>
<table>
	<tr>
		<td align="center">
  			<img src="https://cdn.simpleicons.org/android/3DDC84" height="35" alt="android logo"  />
			<div style="text-align: center;">Android</div>
		</td>
		<td align="center">
  			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg" height="35" alt="kotlin logo"  />
			<div style="text-align: center;">Kotlin</div>
		</td>
	</tr>
	<tr>
		<td align="center">
  			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="35" alt="java logo" />
			<div style="text-align: center;">Java</div>
		</td>
		<td align="center">
            <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/spring/spring-original.svg" height="35" alt="spring logo" />
			<div style="text-align: center;">Spring</div>
		</td>
		<td align="center">
            <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" height="35" alt="react logo" />
			<div style="text-align: center;">React</div>
		</td>
		<td align="center">
  			<img src="https://cdn.simpleicons.org/php/777BB4" height="35" alt="php logo"  />
			<div style="text-align: center;">php</div>
		</td>
	</tr>
	<tr>
		<td align="center">
  			<img src="https://cdn.simpleicons.org/arduino/00979D" height="35" alt="arduino logo"  />
			<div style="text-align: center;">arduino</div>
		</td>
		<td align="center">
  			<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="35" alt="csharp logo"  />
			<div style="text-align: center;">C#</div>
		</td>
	</tr>
</table>


<h4>개발 외</h4>
<table>
	<tr>
		<td align="center">
  			<img src="https://skillicons.dev/icons?i=pr" height="35" alt="adobepremierepro logo"  />
			<div style="text-align: center;">Premier Pro</div>
		</td>
	</tr>
</table>


<br>
<br>

* * *

<h2>💼 일 했어요</h2>
<table>
	<tr>
		<td>
  			<img src="https://image.rocketpunch.com/company/156510/meum_logo_1673438667.png?s=400x400&t=inside" height="35" alt="meum logo"  />

뉴이스트아트 *(A.K.A 믐)*  
<a href="https://meum.me">https://meum.me</a>

> 2020-10-12 ~ 2021-03-31 *(만 5개월)*

```프론트엔드``` 웹사이트 구축, 서버 연동, 유니티 프론트 연동

```vanilla js/css``` ```카카오 간편결제```   
```node.js (라우팅)```

*퇴사 사유: 군복무 이슈*
</td>
	</tr>
	<tr>
		<td>
  			<img src="https://pbs.twimg.com/profile_images/1589499044698603521/DhvNvMX7_400x400.jpg" height="35" alt="stickybomb logo"  />

트리니들 *(A.K.A 스티키밤)*  
<a href="https://stickybomb.kr">https://stickybomb.kr</a>

> 2022-12-01 ~ 2023-02-28 *(만 3개월)*

```백엔드``` 클립 생성 API 서버 구축

```cloudflare/stream``` ```Azure MQ```   
```nest.js``` ```ffmpeg``` ```redis```

*퇴사 사유: 복학 이슈*
</td>
	</tr>
</table>


<br>

* * *

<h2>❤️ 친해지고 싶어</h2>

<img src="https://github.com/xeon-lloyd/xeon-lloyd/assets/24987527/c0a3e242-8d9b-47e3-8aa3-c2e3c075b51d" height="150">

> 간택당하고 싶은 여우여우

물지 않아요

언제든지 제 프로필에 있는 이메일로 연락주세요!<br>
궁금한 점, 밥 한 끼, *~~시비~~* 어떤 것도 환영입니다!

<br>

<h3>저에게 관심있는 분들을 위한 정보제공(TMI)</h3>

- 커피를 마시지 않아요
  - 그래서 카페에 가면 주로 아이스티를 먹고 음료류를 좋아해요
  - 커피 기프티콘을 주시면 사용하지 않고 쌓아둬요..
- 에너지 음료도 마시지 않아요
  - 그럼 뭘로 버티냐고 물어보던데 그냥 정신력으로...
- INTP인데 이제 사회화 된
- 고기를 좋아해요
- 여자친구 없어요
  - 아 있었는데? - 네.. *~~오래전에..~~*
- 수원시에 서식해요
- 군필이에요 *(육군 1사단 21군번, 계산병 했음)*
- 생일은 2001년 6월 24일
- 내 꿈은 돈 많이많이 벌어서 경제적 자유를 얻고 하고싶은 엄청 쓸데없는 프로젝트 만들면서 사는 것

*...etc 생각날 때마다 추가*
